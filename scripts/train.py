"""
train.py — Phase 5: Full training pipeline with text (TF-IDF) + structured features.

Pipeline order:
  1. Preprocessing  (encode, outlier-cap, log-transform structured cols)
  2. Feature Engineering  (9 structured interactions + 100 TF-IDF from title+desc)
  3. Train/test split  (80/20, random_state=42)
  4. Scaling  (RobustScaler fitted on train only)
  5. Feature Selection  (remove low-signal features)
  6. Optuna hyperparameter tuning
  7. Ensemble training  (RF + LightGBM + XGBoost, log-target)

Model bundle saved to models/best_model.pkl
"""
import sys, os
...
from scripts.preprocessing    import (load_raw, step1_drop_duplicates, step2_drop_leakage,
                                      step3_keep_text, step4_fix_formatting,
                                      step5_handle_missing, step6_encode,
                                      step7_handle_outliers, step8_log_transform)
...
def tune_lgbm(Xtr, log_y, n_trials=30):
    ...
    print(f"    Best CV RMSLE: {s.best_value:.4f}  params: {s.best_params}")
...
def run_training():
    ...
    # 1. Load raw + preprocess (text cols kept)
    df = load_raw()
    df = step1_drop_duplicates(df)
    df = step2_drop_leakage(df)
    df = step3_keep_text(df)
    ...
    # 2. Feature engineering (fit TF-IDF on full dataset before split,
    #    then split — this is standard for TF-IDF vocabulary fitting)
    ...
    scaler  = RobustScaler()
    X_tr_s  = pd.DataFrame(scaler.fit_transform(X_tr_r),
                            columns=X_tr_r.columns, index=X_tr_r.index)
    X_te_s  = pd.DataFrame(scaler.transform(X_te_r),
                            columns=X_te_r.columns, index=X_te_r.index)
    ...
    # ── Save ───────────────────────────────────────────────────────────────────
    bundle = {
        "rf": rf_t, "lgbm": lgbm_t, "xgb": xgb_t,
        "log_target": True,
        "type": "rf_lgbm_xgb_log_blend",
    }
